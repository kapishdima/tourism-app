...
class LocationsList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Observer(builder: (context) {
      ...
      return Container(
        padding: const EdgeInsets.only(left: AppSizes.defaulPadding),
        ...
        child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              Location location = locationsStore.includedLocations[index];
              return Observer(builder: (context) {
                return LocationCard(
                  configuration: LocationCardConfiguration(
                      ...
                      rating: location.entity.rating,
                      reviewsCount: location.entity.raviewsCount,
                      type: location.entityType,
                      order: location.order,
                      isStart: index == 0,
                      isFinish:
                          index == locationsStore.includedLocations.length - 1,
                      onCardTap: () {
                        locationsStore.setCurrentLocation(location);
                      },
                      onLinkTap: location.entityType == LocationTypes.winery
                          ? () {
                              Navigator.pushNamed(context, "/winery");
                              wineriesStore.setCurrentWineryId(location.id);
                            }
                          : null,
                      isSelected:
                          locationsStore.currentLocation?.id == location.id),
                );
              });
            },
            itemCount: locationsStore.includedLocations.length),
      );
    });
  }
}
